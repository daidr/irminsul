...
<template>
  <div ref="wrapperRef" class="relative">
    ...
    <!-- Dropdown -->
    <ClientOnly>
      <div v-if="hasOpened" v-show="isOpen"
        ...
        <div class="grid grid-cols-6 gap-1.5 mb-4">
          <button v-for="iconName in BUILTIN_ICON_NAMES" :key="iconName" v-memo="[currentName === iconName]"
            type="button" class="icon-grid-btn w-10 h-10 flex items-center justify-center cursor-pointer"
        ...
        <div class="flex flex-col gap-1.5">
          ...
          <div class="flex items-center gap-2">
            <input type="range" min="0" max="360" :value="currentHue"
              class="flex-1 h-2 appearance-none cursor-pointer hue-slider" @mousedown="isDraggingHue = true"
              @touchstart="isDraggingHue = true" @change="isDraggingHue = false" @input="updateHue" />
            <span class="text-xs text-base-content/60 tabular-nums w-7 text-right">{{ currentHue }}</span>
          </div>
        </div>
      </div>
    </ClientOnly>
