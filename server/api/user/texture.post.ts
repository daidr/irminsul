...
export default defineWebApiHandler(async (event) => {
  ...
  const rateLimitFail = await checkWebRateLimit(event, `web:texture-upload:uid:${user.userId}`, {
    duration: 60_000,
    max: 10,
    delayAfter: 5,
    timeWait: 2_000,
  ...
  try {
    ...
    webError(error instanceof Error ? error.message : "上传失败");
  }
});
